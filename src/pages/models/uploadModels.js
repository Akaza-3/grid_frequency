...
const PickleUploadForm = () => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="m-4 md:m-12 p-6 md:p-8 text-white max-w-lg w-full rounded-lg">
      <div className="mb-4">
      <span className="">
        <label htmlFor="modelName" className="block text-white mb-2">Model Name:</label>
        <input
          type="text"
          id="modelName"
          name="modelName"
      ...
      <div className="mb-4">
        ...
        {inputs.map((input, index) => (
          <div key={index} className="flex flex-wrap items-center gap-2 md:gap-4">
            <label htmlFor={`inputName-${index}`} className="block text-white w-full max-w-xs">Input Name:</label>
            <input
              type="text"
              id={`inputName-${index}`}
              name="name"
            ...
            />
            <label htmlFor={`inputMin-${index}`} className="block text-white w-full max-w-xs">Min Range:</label>
            <input
              type="number"
              id={`inputMin-${index}`}
              name="min"
            ...
            />
            <label htmlFor={`inputMax-${index}`} className="block text-white w-full max-w-xs">Max Range:</label>
            <input
              type="number"
              id={`inputMax-${index}`}
              name="max"
        ...
        <button type="button" onClick={handleAddInput} className="mt-2 md:mt-4 bg-[#575757] text-white py-2 px-4 rounded hover:bg-[#313030]">Add Input Field</button>
      </div>
      <div className="mb-4">
        <label htmlFor="pickleFile" className="block text-white">Pickle File:</label>
        <input
          type="file"
          id="pickleFile"
          name="pickleFile"
      ...
      <button type="submit" className="bg-[#575757] text-white py-2 px-4 rounded hover:bg-[#313030]">Upload Pickle</button>
    </form>
      {graphData === null ? (
        <div style={{ color: "red" }}>
